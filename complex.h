...
namespace cmpx {
    ...
    public:
        ...
        auto& operator *=(Type) noexcept;
        auto operator *(const Complex<Type>&) const noexcept;

        constexpr bool operator ==(const Complex&) const noexcept;
        constexpr bool operator !=(const Complex&) const noexcept;

        constexpr Type realNumber() const noexcept {
            return realNum;
        }
        constexpr Type imaginaryNumber() const noexcept {
            return imaginaryNum;
        }
    ...
    auto Complex<Type>::operator *(const Complex<Type>& c) const noexcept {
        return Complex<Type>{
            realNum * c.realNum, imaginaryNum * c.imaginaryNum
        };
    }
